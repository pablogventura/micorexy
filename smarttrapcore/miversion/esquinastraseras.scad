$fn=32;
module tornillo(){
    color("blue")
rotate([90,0,180]){
cylinder(r1=8/2,r2=4/2,h=4);
cylinder(r=4/2,h=50);
translate([0,0,-9.5])cylinder(r=8/2,h=10);
}
}

//translate([110.7,25,4.8])import("./smartrapcore/XY_bearings - XY_bearings_R.stl");
difference(){
union(){
cube([50,30,15.7]);
translate([10.35+10+2,0,0])cube([27.7,30,15.7+11.1]);
translate([-19,0,1.55])
rotate([-90,0,0])
difference(){
union(){
//translate([53.5,-2.5,0])cylinder(r=24/2,h=30);
translate([26,-14,25/2])rotate([0,3*90,90])cylinder(r1=18/2,r2=9.5/2,h=2.5);
translate([26,-14-21.5,25/2])rotate([0,3*90,90])cylinder(r2=18/2,r1=9.5/2,h=2.5);
translate([0,-44-4,0])cube([57,10,30]);
translate([0,-14,0])cube([55,18,30]);
translate([55.5-14.2,-48,0])cube([10+3.55+14.2,47+5,30]);
translate([0,-44,5])cube([10,44,15]);
}

translate([26,20,25/2])rotate([0,3*90,90])cylinder(r=8/2,h=80);

}}




translate([34.9,50,5.8])rotate([90,0,0])cylinder(r=8.5/2,h=70,$fn=32);

translate([7,12.5,-20])cylinder(r=8/2,h=90,$fn=32);


translate([-7,0,6])tornillo();
translate([-7+25,0,6])tornillo();
translate([-7+25+25,0,6+10])tornillo();
translate([-7+25+25,0,6+10+20])tornillo();
translate([-7,0,6+38.5])tornillo();
translate([-7+25,0,6+38.5])tornillo();
translate([-7+30,4,6+10+10])rotate([0,0,-90])tornillo();
translate([-7+30,4+22,6+10+10])rotate([0,0,-90])tornillo();

}
