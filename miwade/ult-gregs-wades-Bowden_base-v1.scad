...
difference(){
wadeidler();
translate([-24,33,-5])cube([10,6,40]);
}
translate([-21+4,33+3+4+5,14])cube([1,2,26],center=true);
translate([-21+4,33+3-4-5,14])cube([1,2,26],center=true);
*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Visualisation
...
idler_hinge_width=6.3;
idler_end_length=(idler_height-2)+5;
idler_mounting_hole_diameter=m4_diameter+1+0.25;
...
echo("bhmh", mounting_holes)
	...
	translate(large_wheel_translation)
	{
			...
			cube([wade_block_width,wade_block_height-large_wheel_translation[1]+1,wade_block_depth]);
			...
			cylinder(r=4.2/2,h=wade_block_depth*3+elevation+50,center=true,$fn=16);	
			...
			if (render_bracket == false) {
			  translate((mounting_holes==mounting_holes_legacy)?[-3.4,0,-1]:[0,0,0])
			  for (mount=[0:1])
			  {
...
module wadeidler() 
{
	...
	difference()
	{
		...
		translate(idler_axis)
		{
			difference()
			{
				cylinder(h=idler_608_height,r=idler_608_diameter/1.9,center=true,$fn=60);
				for (i=[0,1])
				rotate([180*i,0,0])
